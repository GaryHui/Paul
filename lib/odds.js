...
function candidateTeamName(value) {
  if (!value) return null;
  if (typeof value === "string") return value;
  return value.name || value.team_name || value.full_name || value.country || value.title || value.display_name || null;
}

function directSideNames(event = {}, side) {
  const home = [
    event.home,
    event.homeTeam,
    event.home_team,
  ...
  return (side === "home" ? home : away).map(candidateTeamName).filter(Boolean);
}

function directOddsOrder(match, event = {}) {
  const providerHomeNames = directSideNames(event, "home");
  const providerAwayNames = directSideNames(event, "away");
  const providerHome = providerHomeNames[0] || null;
  const providerAway = providerAwayNames[0] || null;
  const homeIsA = providerHomeNames.some((name) => matchesTeam(match.teamA, name));
  const homeIsB = providerHomeNames.some((name) => matchesTeam(match.teamB, name));
  const awayIsA = providerAwayNames.some((name) => matchesTeam(match.teamA, name));
  const awayIsB = providerAwayNames.some((name) => matchesTeam(match.teamB, name));
  const swapped = homeIsB && awayIsA && !(homeIsA && awayIsB);
  const aligned = homeIsA && awayIsB && !swapped;
  return {
    fixtureHome: match.teamA.name,
    fixtureAway: match.teamB.name,
    providerHome,
    providerAway,
    swapped,
    confidence: aligned ? "matched" : swapped ? "matched-reversed" : "unverified"
  };
}
...
function summarizeDirectOdds(provider, match, event, odds, extra = {}) {
  const order = directOddsOrder(match, event);
  const rawOdds = {
    home: decimalOdds(odds.home),
    draw: decimalOdds(odds.draw),
    away: decimalOdds(odds.away)
  };
  const normalizedOdds = order.swapped
    ? { home: rawOdds.away, draw: rawOdds.draw, away: rawOdds.home }
    : rawOdds;
...
async function fetchBsdOdds(match) {
  ...
  const order = directOddsOrder(match, detail);
  const homeCoach = compactCoach(detail.home_coach || event.home_coach);
  const awayCoach = compactCoach(detail.away_coach || event.away_coach);
  const homeForm = compactForm(detail.home_form);
  const awayForm = compactForm(detail.away_form);
  const intelligence = {
    source: "bsd",
    ...
    sideOrder: order,
    coaches: {
      home: order.swapped ? awayCoach : homeCoach,
      away: order.swapped ? homeCoach : awayCoach
    },
    unavailablePlayers: compactUnavailable(detail.unavailable_players || event.unavailable_players),
    form: {
      teamA: order.swapped ? awayForm : homeForm,
      teamB: order.swapped ? homeForm : awayForm
    },
